...
func (e *ExecutionEngine) RunRecall(ctx context.Context, taskID, recallNodeID string, upstreamNodeIDs []string, goal string, engine ProbeInferenceEngine) (RecallResult, error) {
	...
	// ADR-0094: Fan-reduce shortcut deleted. DocGen tasks now use
	// RecallPolicy: "skip" — Recall is never injected, and Sectioned
	// Synthesis handles decomposition natively with embedding-based
	// chunk dedup. For non-docgen tasks, deterministic structural
	// compaction (buildCompactedRecallContext) handles budget reduction.
