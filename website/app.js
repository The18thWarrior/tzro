...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function resetVariantASim() {
    ...
    vAConsole.innerHTML = '<div class="log-line text-muted">Awaiting topological compilation triggers...</div>';
  ...
  function runVariantASim() {
    ...
    appendConsoleLine("Compiling abstract graph definition with Kahn Sorter...", "cyan");
    
    // Phase 1: Planning / Strategist runs
    vASimIntervals.push(setTimeout(() => {
      nodePlanner.className = 'sim-node running';
      appendConsoleLine("Strategist (Gemini Cloud) called ONCE. Compiling Abstract execution blueprint...", "log-line");
    }, 1000));

    vASimIntervals.push(setTimeout(() => {
      nodePlanner.className = 'sim-node completed';
      appendConsoleLine("Strategic compilation complete. Sorted levels constructed programmatically.", "success");
      appendConsoleLine("Level sequence mapping: Level 0 [node_01, node_02] (Parallel) -> Level 1 [node_03] (Dependent)", "cyan");
    }, 2500));
    ...
    vASimIntervals.push(setTimeout(() => {
      ...
      appendConsoleLine("Level 0 goroutines spawned. Dispatching concurrent tasks in parallel.", "cyan");
      appendConsoleLine("Executing node_01 (archive_files) with GBNF grammar constraints...", "log-line");
      appendConsoleLine("Executing node_02 (fetch_user_details) as WASM sandboxed micro-skill...", "log-line");
    }, 4500));

    vASimIntervals.push(setTimeout(() => {
      node01.className = 'sim-node completed';
      appendConsoleLine("[Level 0: Goroutine 1] node_01 completed. Output: archive_report.zip generated.", "success");
    }, 6500));

    vASimIntervals.push(setTimeout(() => {
      node02.className = 'sim-node completed';
      appendConsoleLine("[Level 0: Goroutine 2] node_02 completed. Output: user profiles compiled (Size: 4.8KB).", "success");
    }, 7200));

    // Phase 3: Level 1 executes
    vASimIntervals.push(setTimeout(() => {
      appendConsoleLine("All Level 0 dependency edges satisfied. Unblocking Level 1.", "cyan");
      node03.className = 'sim-node running';
      appendConsoleLine("Executing node_03 (send_team_alert) using Stdio MCP gateway. Mapping parameters forwarded from node_01/node_02...", "log-line");
    }, 8500));

    vASimIntervals.push(setTimeout(() => {
      node03.className = 'sim-node completed';
      appendConsoleLine("[Level 1] node_03 completed. Notification pushed via Stdio Slack MCP server.", "success");
    }, 10500));

    // Terminal synthesis node
    vASimIntervals.push(setTimeout(() => {
      appendConsoleLine("Terminal synthesis node initialized. Compiling summary block...", "cyan");
    }, 11500));

    vASimIntervals.push(setTimeout(() => {
      appendConsoleLine("TASK DEMO EXECUTION COMPLETE. Status: success. Cloud API calls: 1. Local tool dispatches: 3. Telemetry Stream: closed.", "success");
      if (vAHorizonPulse) {
        vAHorizonPulse.classList.remove('pulsing');
        vAHorizonPulse.style.transform = 'scaleX(0)';
        vAHorizonPulse.style.opacity = '0';
  ...
  const compactedPayloadDisplay = document.getElementById('compacted-payload-display');
  ...
  presetBtns.forEach(btn => {
    btn.addEventListener('click', (e) => handlePresetSelection(e.currentTarget));
  });
  ...
  function runVariantBSim() {
    ...
    vBSimIntervals.push(setTimeout(() => {
...
func main() {
  ...
  function resetVariantCSim() {
    ...
    vCSimIntervals = [];
    dxConsole.innerHTML = '<div class="console-line text-muted">Initialize the pipeline configuration to run...</div>';
  ...
  function runVariantCSim() {
    ...
    appendDxConsole("Initializing database engine layer... [Success]", "success");
    appendDxConsole("Registering execution hooks with GlobalEngine...", "console-line");
    ...
    if (piiActive) appendDxConsole("[Hook Enabled] AfterNode output mutator registered.", "warning");
    if (guardActive) appendDxConsole("[Hook Enabled] BeforeNode safety blocker registered.", "warning");
    if (hitlActive) appendDxConsole("[Hook Enabled] BeforeLevel blocking supervisor gate registered.", "warning");

    appendDxConsole("Compiling Execution Graph (Task: t_hook_run)...", "console-line");

    // Phase 1: Level 0 executes (Safe Nodes)
    vCSimIntervals.push(setTimeout(() => {
      appendDxConsole("Kahn sorting complete. Running Level 0 steps...", "console-line");
      appendDxConsole("[Level 0 Node: node_01] Invoking tool 'fetch_user_details'...", "console-line");
    }, 1200));

    vCSimIntervals.push(setTimeout(() => {
      let rawResult = '{"status": "success", "username": "jp", "credentials": "SSN_SECRET_VALUE"}';
      appendDxConsole(`[Level 0 Node: node_01] Tool raw output: ${rawResult}`, "console-line");
      
      if (piiActive) {
        appendDxConsole("[Hook Triggered: AfterNode] Safety check. SSN credentials detected in raw response.", "warning");
        rawResult = '{"status": "success", "username": "jp", "credentials": "[REDACTED]"}';
        appendDxConsole(`[Hook Mutated Output]: ${rawResult}`, "success");
      }
      appendDxConsole("[Level 0 Node: node_01] Saved securely to database state checkpoints.", "success");
    }, 2800));

    // Phase 2: Level 1 executes
    vCSimIntervals.push(setTimeout(async () => {
      appendDxConsole("Unblocking Level 1. Running level checks...", "console-line");
      
      if (hitlActive) {
        appendDxConsole("[Hook Triggered: BeforeLevel] Human-In-The-Loop hook blocks execution.", "warning");
        appendDxConsole("Sent Sentinel Error: ErrTaskPaused. Halting thread and yielding execution.", "error");
        ...
        hitlPrompt.style.display = "none";
        if (approved) {
          appendDxConsole("Durable task resumed successfully from checkpoint. Level 1 unblocked by human supervisor.", "success");
          vCRunBtn.textContent = "Resuming Task...";
          continueLevel1Execution(guardActive);
        } else {
          appendDxConsole("Task aborted by human supervisor. State reset.", "error");
          vCRunBtn.disabled = false;
  ...
  function continueLevel1Execution(guardActive) {
    appendDxConsole("[Level 1 Node: node_02] Intercepting action 'delete_all_records'...", "console-line");
    
    vCSimIntervals.push(setTimeout(() => {
      if (guardActive) {
        appendDxConsole("[Hook Triggered: BeforeNode] Safety violation! Destructive tool call blocked.", "error");
        appendDxConsole("[Hook Output: ActionSkip] Automatically skipping tool run, propagating skip state downstream.", "warning");
        appendDxConsole("[Level 1 Node: node_02] Marked as [skipped] in SQLite checkpoints.", "success");
      } else {
        appendDxConsole("[Level 1 Node: node_02] Invoking tool 'delete_all_records' directly without guardrails!", "error");
        appendDxConsole("[Level 1 Node: node_02] Database records deleted completely.", "success");
      }
  ...
  function finalizeLevel1Run() {
    vCSimIntervals.push(setTimeout(() => {
      ...
      appendDxConsole("Kahn executor complete. Running synthesis summary...", "console-line");
    }, 1200));

    vCSimIntervals.push(setTimeout(() => {
      appendDxConsole("Task completed. All execution hooks disposed securely.", "success");
      vCRunBtn.disabled = false;
