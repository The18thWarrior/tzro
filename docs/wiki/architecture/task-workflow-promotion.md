...
*   **Trigger**: The prompt implies temporal gaps, deferrals, cron triggers, or asynchronous wait states (e.g. *"Wait 3 days"*, *"Run every Tuesday at 9 AM"*, *"Wait until the lead accepts the invite"*).
...
*   **Trigger**: The prompt requests explicit Human-in-the-Loop (HITL) validation checkpoints between heavy operations (e.g. *"Perform a dry run and wait for my sign-off before executing the Salesforce writes"*).
