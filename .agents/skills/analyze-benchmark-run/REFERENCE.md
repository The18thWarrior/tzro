...
3. *Example*: Instead of having 5 distinct tools (`get_user_name`, `get_user_email`, `get_user_phone`, etc.), combine them into a single `get_user_profile` tool that takes a mask. This drastically reduces the planner's registry search space.
